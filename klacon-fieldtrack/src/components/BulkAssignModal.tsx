...
export function BulkAssignModal({ 
  ...
  const [service, setService] = useState(state.servicesList[0] || '');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative bg-white rounded-2xl w-full max-w-sm shadow-2xl flex flex-col"
      >
        <div className="flex items-center justify-between p-5 border-b border-neutral-100">
          <h3 className="text-lg font-bold text-neutral-900 tracking-tight">Atribuir em Massa</h3>
          <button onClick={onClose} className="text-neutral-400 hover:text-neutral-900 p-1.5 rounded-full hover:bg-neutral-100 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-5">
          <div className="flex flex-col gap-2">
            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Serviço</label>
            <CustomSelect
              value={service}
              onChange={setService}
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Empreiteira</label>
            {!isNewContractor ? (
              <CustomSelect
                value={contractor}
                onChange={(val) => {
              ...
              >
                <input 
                  ...
                  className="flex-1 bg-white border border-neutral-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent transition-all shadow-sm"
                />
                <button 
                  type="button"
                  onClick={() => setIsNewContractor(false)}
                  className="px-4 py-3 bg-neutral-100 hover:bg-neutral-200 rounded-xl text-neutral-600 transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </motion.div>
          ...
          <p className="text-[11px] leading-relaxed text-neutral-500 mt-2 bg-neutral-50 p-3 rounded-lg border border-neutral-100">
          ...
          <div className="flex gap-3 pt-3">
            <button 
              type="button" 
              onClick={onClose}
              className="flex-1 py-3 rounded-xl text-sm font-bold text-neutral-600 bg-neutral-100 hover:bg-neutral-200 transition-colors"
            >
              Cancelar
            </button>
            <button 
              ...
              className="flex-1 py-3 rounded-xl text-sm font-bold text-neutral-900 bg-teal-500 hover:bg-teal-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
            >
              Aplicar a Todos
            </button>
          </div>
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className={cn(
          "w-full flex items-center justify-between bg-white border rounded-xl px-4 py-3 text-sm transition-all shadow-sm",
          isOpen ? "border-teal-600 ring-2 ring-teal-600/20" : "border-neutral-200 hover:border-neutral-300",
              ...
              {options.map(opt => (
                <button
                  ...
                  className={cn(
                    ...
                    value === opt 
                      ? "bg-teal-50 text-amber-900 font-medium" 
                      : "text-neutral-700 hover:bg-neutral-50"
                  )}
                >
                  <span className="truncate">{renderOption ? renderOption(opt) : opt}</span>
                  {value === opt && opt !== 'NEW' && <Check className="w-4 h-4 text-teal-600 shrink-0" />}
                </button>
              ))}
            </div>
          </motion.div>
