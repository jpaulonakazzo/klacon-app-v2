  ...
  return (
    <div className="p-4 flex flex-col gap-6">
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm">
        <h2 className="text-lg font-bold text-neutral-800 mb-4">Progresso Geral da Obra</h2>
          ...
          <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="w-full max-w-[200px] aspect-square relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-3xl font-bold text-neutral-800 tracking-tight">{overallProgress}%</span>
                <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider">Concluído</span>
              </div>
            </div>
              ...
              {dashboardData.map(item => (
                <div key={item.name} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-neutral-400 w-8 text-right">{Math.round((item.value / totalServices) * 100)}%</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="flex flex-col gap-2">
        <h2 className="text-2xl font-bold text-neutral-900">Selecione o Local</h2>
        <p className="text-sm text-neutral-500">Escolha o pavimento e em seguida o apartamento ou área para vistoria.</p>
      </div>
        ...
        {FLOORS.find(f => f.id === selectedFloor)?.locations.map(loc => {
          ...
          return (
            <button
              ...
              className="bg-white border border-neutral-200 rounded-xl p-4 flex flex-col items-center justify-center gap-2 aspect-square active:scale-95 transition-transform hover:shadow-sm"
            >
              <span className="text-2xl font-bold text-neutral-800">{loc}</span>
              <div className="w-full flex flex-col gap-1 mt-1">
                ...
                <span className="text-[10px] text-neutral-400 font-medium tracking-wider uppercase text-center">{percent}% Concluído</span>
              </div>
            </button>
          );
        })}
