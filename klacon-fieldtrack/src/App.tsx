...
import { Home, Search, Bell, Map, Download, Wifi, WifiOff, Save, Plus, Users } from 'lucide-react';
...
export default function App() {
  const { state, updateService, markNotificationRead, updateNotificationSettings, exportCSV, addGlobalService, assignContractorBulk } = useAppStore();
  ...
  return (
    <div className="flex flex-col h-screen bg-neutral-100 text-neutral-900 font-sans">
      {/* Header */}
      <header className="bg-white border-b border-neutral-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl tracking-tight text-neutral-800 leading-none flex items-center gap-1.5 -ml-1">
              <span className="font-light lowercase">klacon</span>
              <span className="font-bold text-slate-500 uppercase text-[10px] tracking-widest mt-1.5">Fieldtrack</span>
            </h1>
          </div>
          <div className="flex items-center gap-2 mt-1.5">
            {isOnline ? (
              <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-100 px-1.5 py-0.5 rounded border border-emerald-200">
                <Wifi className="w-3 h-3" /> Online
              </span>
            ) : (
              <span className="flex items-center gap-1 text-[10px] font-bold text-red-700 bg-red-100 px-1.5 py-0.5 rounded border border-red-200">
                <WifiOff className="w-3 h-3" /> Offline
              </span>
            )}
            <span className="flex items-center gap-1 text-[10px] font-bold text-neutral-600 bg-neutral-100 px-1.5 py-0.5 rounded border border-neutral-200">
      ...
      {/* Bottom Navigation */}
      <nav className="bg-white border-t border-neutral-200 fixed bottom-0 w-full flex justify-around items-center h-16 px-2 z-10 pb-safe">
        <NavButton 
